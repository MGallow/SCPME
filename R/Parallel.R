...
CVP_ADMM = function(X, Y = NULL, A = diag(ncol(X)), B = diag(ncol(X)), C = diag(ncol(X)), 
    lam = 10^seq(-2, 2, 0.2), alpha = 1, tau = 10, rho = 2, mu = 10, tau.rho = 2, iter.rho = 10, 
    crit = c("ADMM", "loglik"), tol.abs = 1e-04, tol.rel = 1e-04, maxit = 1000, adjmaxit = NULL, 
    ...
    CV = foreach(k = 1:K, .packages = "shrink", .combine = "cbind", .inorder = FALSE) %dopar% 
        {
            ...
            CVP_ADMMc(X_train = X.train, X_valid = X.valid, Y_train = Y.train, Y_valid = Y.valid, 
                A = A, B = B, C = C, lam = lam, alpha = alpha, tau = tau, rho = rho, mu = mu, 
                tau_rho = tau.rho, iter_rho = iter.rho, crit = crit, tol_abs = tol.abs, 
                tol_rel = tol.rel, maxit = maxit, adjmaxit = adjmaxit, crit_cv = crit.cv, 
